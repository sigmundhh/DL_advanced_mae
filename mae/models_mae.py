...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_loss(self, imgs, pred, mask, alpha, beta):
        """
        imgs: [N, 3, H, W]
        pred: [N, L, p*p*3]
        ...
        loss = alpha * (loss * (1-mask)).sum() / (1-mask).sum() + beta * (loss * mask).sum() / mask.sum()  # mean loss on removed patches
...
def mae_vit_base_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=16, num_heads=12, decoder_num_heads=16,
...
def smaller_mae_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=14, embed_dim=192, depth=8, num_heads=8,
...
"""
# The following will be unnecessary since we only will call the smallest architecture with 
# differnet decoder/encoder depth/dimensions.
mae_vit_large_patch16 = mae_vit_large_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
mae_vit_huge_patch14 = mae_vit_huge_patch14_dec512d8b  # decoder: 512 dim, 8 blocks
our_mae = smaller_mae_dec512d8b #
"""
